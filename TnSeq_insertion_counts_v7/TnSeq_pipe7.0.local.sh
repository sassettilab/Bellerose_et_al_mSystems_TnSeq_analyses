...
# TnSeq_pipe7.0.sh is TnSeq_pipe6.0.sh modified to process 75-base reads.
#
# requires edit_fastqs_v5.0.pl to account for CASAVA 1.8+ read IDs
# REQUIRES FULL PATH NAME TO INPUT FILES (v5.0.2)
# expects BWA indexes in local subdirectioy --> ./bwa_indexes_tb
#
# As for TnSeq_pipe.6.0.sh
#	requires barcode_use_v1.0.pl, get_raw_counts_piped.pl in working directory
#	along with edit_fastqs_v5.0.pl and barcodes4bwa_v2.0
#
...
if [ $# -eq 0 ]
then
	echo "usage: TnSeq_pipe7.0.sh <reads1.fastq|reads1.fastq.gz> <reads2.fastq|reads2.fastq.gz><basename for output>"
...
perl ../edit_fastqs_v5.0.pl $FILETAG\_1.fastq $FILETAG\_2.fastq $3
echo '### aligning...'
bwa aln -q 5 -l 32 -k 2 -o 1 -t 4 NC_018143.1.fa $3\_reads.txt > $3\_reads.sai
bwa aln -q 5 -l 32 -k 2 -o 1 -t 4 NC_018143.1.fa $3\_mates.txt > $3\_mates.sai
