...
#	27Feb2015
#
# version 5.0 modified for 75 base reads; writes 22-base mate reads
# also minor bug fix in mate clipping; only searches for barcode AFTER 24 nucleotides
#	(avoids "finding" a gratuitous barcode sequence in beginning of read)
# 30Aug2018
#
use strict;
...
while (my $seqID = <READS>) {
	...
	if ($seq =~ /^[GATCN]{21,26}GTG([GATC]{2}A[GATC]{2}A[GATC]{3})TGG/) { # $1 = barcode
		$mate_clip = index($seq, $1, 24) + 21; # index offset = 24
	...
	my $clip = substr($seq, $mate_clip, 22);
	if (length($clip) < 22) { # read too short; this should not happen (often)
		$junk_mates++;
		print OUT4 ">$count\n$seq\n";
		<MATES>;
		<MATES>;
	...
	my $qual = <MATES>;
	chomp $qual;
	push @mate, substr($qual, $mate_clip, 22);
